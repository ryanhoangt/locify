...
class WarmupBenchmark:
    ...
    def reset(self):
        # Delete tags cache directory
        tags_cache_dir = Path(self.repo_path) / '.cache.tags.v3'
        if tags_cache_dir.exists():
            print('Deleting tags cache directory...')
            shutil.rmtree(tags_cache_dir)
            # Check if cache was deleted
            if tags_cache_dir.exists():
                raise Exception('Failed to delete cache directory')
            else:
                print('Cache directory deleted successfully')
        self.strategy_with_cache = FullMapStrategy(root=self.repo_path)
