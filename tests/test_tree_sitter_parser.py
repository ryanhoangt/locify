...
"""
    ...
    tags = parser.get_tags_from_file(
        str(python_file.absolute()),
        str(python_file.relative_to(tmp_path))
    )
...
def test_get_tags_from_nonexistent_file(parser):
    tags = parser.get_tags_from_file(
        "/path/that/does/not/exist/file.py",
        "file.py"
    )
    assert tags == []
...
def test_get_tags_from_unsupported_file(parser, tmp_path):
    ...
    tags = parser.get_tags_from_file(
        str(unsupported_file.absolute()),
        str(unsupported_file.relative_to(tmp_path))
    )
    assert tags == []
...
"""
    ...
    tags = parser.get_tags_from_file(
        str(java_file.absolute()),
        str(java_file.relative_to(tmp_path))
    )
...
def test_get_tags_from_empty_file(parser, tmp_path):
    ...
    tags = parser.get_tags_from_file(
        str(empty_file.absolute()),
        str(empty_file.relative_to(tmp_path))
    )
    assert tags == []